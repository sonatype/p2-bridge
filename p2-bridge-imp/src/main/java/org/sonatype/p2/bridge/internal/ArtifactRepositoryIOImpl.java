...
import org.sonatype.p2.bridge.ArtifactRepositoryIO;
import org.sonatype.p2.bridge.model.InstallableArtifact;
import org.sonatype.p2.bridge.model.InstallableArtifactProperty;

public class ArtifactRepositoryIOImpl
    implements ArtifactRepositoryIO
{
                       ...
                       final Map<String, String> properties )
    {
        try
        {
            ...
            if ( provider == null )
            {
                throw new RuntimeException( "Cannot write artifact repository as there is no provisioning agent provider" );
            }
            ...
            if ( manager == null )
            {
                throw new RuntimeException( "Cannot write artifact repository as artifact repository manager coud not be created" );
            }
            ...
            {
                throw new RuntimeException( "Cannot write artifact repository as created repository is not of expected type (SimpleArtifactRepository)" );
            }
        ...
        throws ProvisionException
    {
        for ( final InstallableArtifact artifact : artifacts )
        {
            final IArtifactKey artifactKey =
                repository.createArtifactKey( artifact.getClassifier(), artifact.getId(), Version.parseVersion( artifact.getVersion() ) );
            ...
            {
                throw new RuntimeException( "Cannot write artifact repository as created artifact descriptor is not of expected type (ArtifactDescriptor)" );
            }
            ...
            if ( output == null )
            {
                throw new RuntimeException( String.format( "Cannot write artifact repository as artifact %s could not be copied", artifact.getPath() ) );
            }
            output = new BufferedOutputStream( output );
            try
            {
                FileUtils.copyStream( new BufferedInputStream( new FileInputStream( artifactFile ) ), true, output, true );
            }
            catch ( final Exception e )
            {
                throw new RuntimeException( String.format( "Cannot write artifact repository as artifact %s could not be copied", artifact.getPath() ), e );
            }
        }
    }
